...
## 20. P0 Development Readiness Addendum

**Status:** Required before application development begins  
**Source:** `FINAL_GAP_ANALYSIS.md`, `SYSTEM_ARCHITECTURE.md`, `API_DESIGN.md`, and `DEVELOPMENT_ROADMAP.md`

### 20.1 Canonical MVP Scope

The MVP is limited to the owner, workshop, and report trust loop. Dealer, insurer, EV, inventory, invoicing, OCR, telematics, partner API, and fleet capabilities remain Version 2 or Version 3 unless explicitly pulled into MVP by a new product decision.

MVP includes:

- Owner onboarding, OTP login, profile, and PDPA consent.
- Vehicle creation through a claim workflow, not automatic trusted ownership.
- Vehicle ownership roles: primary owner, co-owner, watcher, previous owner.
- Core passport records: service, repair, insurance, road tax, loan, documents, reminders.
- Workshop onboarding, organization membership, job cards, and verified service publication.
- Passport report generation as an asynchronous job with immutable snapshot and PDF output.
- Report sharing through revocable, expiring share links.
- Pay-per-report and report-credit foundation with payment webhook reconciliation.
- Admin workflows for vehicle claims, organization verification, record disputes, duplicate vehicles, and report revocation.
- Public plate teaser with minimized output and abuse controls.

### 20.2 Required P0 Product Flows

The following flows are required before MVP development starts:

- Vehicle claim: submit claim, upload proof, pending review, approve, reject, duplicate review.
- Ownership transfer: start transfer, accept, reject, expire, dispute, admin override.
- Consent grant: grant access, view active grants, revoke grant, audit grant usage.
- Record dispute: dispute a record, upload evidence, admin resolve, mark disputed/revoked/superseded.
- Report generation: request report, check job status, retry failure, release/refund credit on failure.
- Report sharing: create share, open public snapshot, revoke share, expire share, log views.
- Payment: create checkout, wait for signed webhook confirmation, grant entitlement, reconcile failure/refund.
- Organization staff invitation: invite member, accept invite, assign role, revoke invite.
- PDPA operation: request data export, deactivate account, apply retention/anonymisation rules.

### 20.3 Required P0 Business Rules

- No user receives trusted ownership solely by entering a plate number.
- Dealer, insurer, workshop, and public access must use explicit consent, purchased report snapshots, or minimized teaser data.
- Public reports must never expose raw IC/passport numbers, full VIN, policy numbers, loan account numbers, owner identity, or raw private documents.
- Workshop-verified records become immutable after publication; corrections use dispute, revocation, or superseding records.
- Credits are consumed atomically and must be reversible when report generation fails.
- Payment webhooks must be signed, idempotent, and replay-protected before entitlements are granted.
- Public plate checks must be rate-limited and return coarse information only.
- All sensitive actions must create audit events.

### 20.4 Required P0 Data Models

The database specification must include these P0 models before implementation:
...
- `rate_limit_events` or `abuse_events`

### 20.5 Required P0 Algorithms

The following algorithms must be specified before UI implementation:

- Vehicle completeness score.
- Data confidence score.
- Service regularity rating.
- Accident, flood, loan, insurance, and road-tax flags.
- Verified vs owner-declared weighting.
- Report masking and snapshot rules.
- Entitlement rules for subscriptions, report credits, storage quotas, staff seats, and messaging quotas.

### 20.6 Version Boundary Confirmation

- **MVP:** Owner + workshop + report trust loop, including claim, consent, dispute, report, payment, audit, and admin foundations.
- **Version 2:** Dealer module, EV module, Workshop Professional, inventory, invoicing, WhatsApp campaigns, OCR, MyEG/Puspakom pilots.
- **Version 3:** Insurer module, bank integrations, partner APIs, telematics, fleet management, advanced analytics.
- **Future:** Marketplace, AI automation, government-grade verification, data clean room, multi-region enterprise platform.
